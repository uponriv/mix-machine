...
class VMBaseTestCase(unittest.TestCase):
    ...
    def check1(self, regs={}, memory={}, devs={}, startadr=0, diff={}, cycles=0,
               message=None):
        self.assertEqual(self.exec1(regs, memory, devs, startadr), diff,
                         message)
        self.assertEqual(self.cycles, cycles, message)

    def check_hlt(self, regs={}, memory={}, devs={}, startadr=0, diff={},
                  cycles=0, message=None):
        self.assertEqual(self.exec_hlt(regs, memory, devs, startadr), diff,
                         message)
        self.assertEqual(self.cycles, cycles, message)
...
def _memory_value(adr):
    ...
    return [sign] + [(u_num >> shift) & mask for shift in
                     range(24, -1, -6)]  # 24 = 6 * (5-1)
