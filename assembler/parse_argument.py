...
class ArgumentParser:
    ...
    def try_number(self):
        if self.get() is not None and 1 <= len(
                self.get()) <= 10 and self.get().isdigit():
            return int(self.get())
        else:
            return None

    def try_defined_symbol(
            self):  # Program algorithm is so that difference of symbols destroied.
    ...
    def try_future_symbol(
            self):  # Assemble - there parsed arguments of instructions
    ...
    def try_literal(self):
        ...
        if res is None:
            ...
            if self.get() != "=":
                ...
                if res == 0:
                    try:
                        if self.line.argument[
                            1] == '-':  # (self.line.argument[0] = "=")!!
                            return_tuple = (0, -1)
                        else:
                            return_tuple = (0, +1)
                    except:
