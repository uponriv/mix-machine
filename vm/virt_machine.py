...
class VMachine:
    ...
    def __getitem__(self, x):
        ...
        if isinstance(x, slice):  # slice, vm[2000:2:4] = ...
            ...
            left = 0
            right = 5
        return (self.memory[item] if isinstance(item, int) else self.reg(item))[
               left:right]
    ...
    def is_writeable_set(self, _set):
        return len(_set & (self.locked_cells[self.W_LOCKED] | self.locked_cells[
            self.RW_LOCKED])) == 0
    ...
    def lock_cells(self, mode, add=None, sub=None):
        ...
        if self.lock_hook is not None and self.locked_cells[mode] != old:
            self.lock_hook("rw" if mode == self.RW_LOCKED else "w", old,
                           self.locked_cells[mode])
        ...
        for dev in self.devices.values():
            ...
            if unlock is not None:
                ...
                if unlock[0] == 'w':
                    ...
                    return  # ioc busy
                # unlock memory
                self.lock_cells(mode,
                                sub=set(range(unlock[1][0], unlock[1][1] + 1)))
