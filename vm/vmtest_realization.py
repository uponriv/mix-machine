...
class VMTesting:
    ...
    def load(self, mega, devs={}, hook=None):
        ...
        if mega.get("W_LOCKED") is not None: self.vm.locked_cells[
            self.vm.W_LOCKED] = set(mega["W_LOCKED"])
        if mega.get("RW_LOCKED") is not None: self.vm.locked_cells[
            self.vm.RW_LOCKED] = set(mega["RW_LOCKED"])
    ...
    def state(self):
        """Returns MEGA hash"""
        mega = dict(
            [(i, self.vm[i].word_list[:]) for i in range(self.vm.MEMORY_SIZE)])
